...
- 按用户要求：两个 HTML 详细报告从本地 ~/report/ 移入 wiki repo
  ...
  - reports/fixanything/index.html（FixAnything 演讲报告）
- 本地 ~/report/physgs、~/report/fixanything 已删除，不再留多副本
- index.md 两个概念条目补充 HTML 路径
...
- 演讲配套：Google Slides "Aug 31, 2026 - checkin (FixAnything)"（9 页，复用 checkin 模板布局）+ 详细 HTML 报告（reports/fixanything/index.html，dark-themed SVG 架构图）
...
- Raw 源归档：raw/papers/physgs-arxiv-2511-18570.md（sha256 frontmatter）、raw/articles/physgs-website.md（8/24 已有）
- 更新 physical-property-estimation-from-scans.md：2.3 节改为摘要 + 指向 [[physgs]]，sources 补论文 raw
- 演讲配套：Google Slides "Aug 31, 2026 - checkin"（9 页，复用 8/24 checkin 模板布局）+ 详细 HTML 报告（reports/physgs/index.html，dark-themed SVG 架构图）
- 更新 index.md（Concepts 新增 physgs，Total pages 16 → 17）
